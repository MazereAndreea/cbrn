...
CONF_THRESHOLD  = 0.15   # detection confidence for YOLO
CRIT_THRESHOLD  = 0.3    # threshold for body-analysis decisions
WORK_OFFSET_M   = 0.5
...
def classify_body_position_coco(kp_conf):
    """
    Classify lying body orientation from COCO 17-keypoint confidences.
...
def select_injection_zone_coco(kp_xy_norm, kp_conf, body_pos, frame_w, frame_h):
    """
    Returns (zone_type, px, py, confidence) or None.
    kp_xy_norm: array shape (17, 2) — normalized [0,1].
    """
    T = CRIT_THRESHOLD
...
class YoloDetector(Node):
    def __init__(self):
        ...
        csv_path = os.path.join(log_dir, f'metrics_yolo_{ts}.csv')
        ...
        self.get_logger().info(f'YOLO detector ready. Metrics CSV → {csv_path}')
    ...
    def _img_cb(self, msg):
        ...
        for r in results:
            ...
            kpts  = r.keypoints.xyn.cpu().numpy()[0]   # (17, 2) normalized
            confs = (r.keypoints.conf.cpu().numpy()[0]
                     if r.keypoints.conf is not None else np.zeros(17))
            ...
            kp_msg.kp_x           = kpts[:, 0].tolist()
            kp_msg.kp_y           = kpts[:, 1].tolist()
            kp_msg.kp_conf        = confs.tolist()
            ...
            csv_det  = True
            csv_bw, csv_bh = bw, bh
            csv_crit  = [float(confs[i]) for i in CRITICAL_IDX]
            csv_above = int(np.sum(confs > CRIT_THRESHOLD))
            ...
            # Body position
            body_pos = classify_body_position_coco(confs)
            csv_bpos = body_pos
            inj_msg.body_position = body_pos

            # Injection zone
            zone = select_injection_zone_coco(kpts, confs, body_pos, w, h)
            if zone:
            ...
            for i, (kx, ky) in enumerate(kpts):
                if confs[i] > CRIT_THRESHOLD:
                    cv2.circle(frame, (int(kx * w), int(ky * h)), 4, (0, 0, 255), -1)
            for p1, p2 in SKELETON:
                if confs[p1] > CRIT_THRESHOLD and confs[p2] > CRIT_THRESHOLD:
                    pt1 = (int(kpts[p1][0] * w), int(kpts[p1][1] * h))
                    pt2 = (int(kpts[p2][0] * w), int(kpts[p2][1] * h))
                    cv2.line(frame, pt1, pt2, (0, 255, 255), 2)

            # Centre target for legacy robot_controller
            target_msg.x = float((cx_bb / w) - 0.5)
            target_msg.y = float(cy_bb)
            target_msg.z = float(dist_m)
            break  # first detected person only

        cv2.putText(frame, f'YOLO | {inf_ms:.1f} ms | {csv_bpos}',
        ...
        self._csv.writerow(
            [time.time(), 'yolo11', int(csv_det), f'{inf_ms:.2f}',
